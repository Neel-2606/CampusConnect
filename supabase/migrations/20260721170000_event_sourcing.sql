...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (NEW.id, v_event_type, v_payload, NOW());
    RETURN NEW;
  ...
  ELSIF TG_OP = 'DELETE' THEN
    ...
    INSERT INTO event_store (aggregate_id, event_type, payload, timestamp)
    VALUES (OLD.id, v_event_type, v_payload, NOW());
    RETURN OLD;
  END IF;
  RETURN NULL;
END;
...
WITH ranked_events AS (
  SELECT 
    ...
    payload,
    timestamp,
    ROW_NUMBER() OVER(PARTITION BY aggregate_id ORDER BY timestamp DESC) as rn
  FROM event_store
)
SELECT 
  aggregate_id,
...
-- 5. RLS for event_store and view
ALTER TABLE event_store ENABLE ROW LEVEL SECURITY;
-- By default, event_store is only accessible by admins/service role.
-- No public policies created to keep audit logs secure.
